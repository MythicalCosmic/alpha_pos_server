...
A_OPENAIMODEL="$(keep "$A_ENV" OPENAI_MODEL)"; A_OPENAIMODEL="${A_OPENAIMODEL:-gpt-5.6-luna}"
A_OPENAIREASONING="$(keep "$A_ENV" OPENAI_REASONING_EFFORT)"; A_OPENAIREASONING="${A_OPENAIREASONING:-low}"
A_OPENAISEED="$(keep "$A_ENV" OPENAI_SEED)"; A_OPENAISEED="${A_OPENAISEED:-7}"
A_AITEMPERATURE="$(keep "$A_ENV" AI_TEMPERATURE)"; A_AITEMPERATURE="${A_AITEMPERATURE:-0}"
A_WEBCONCURRENCY="$(keep "$A_ENV" WEB_CONCURRENCY)"; A_WEBCONCURRENCY="${A_WEBCONCURRENCY:-3}"
...
A_AIDEADLINE="$(keep "$A_ENV" AI_REQUEST_DEADLINE_SECONDS)"; A_AIDEADLINE="${A_AIDEADLINE:-150}"
...
A_SDMAX="$(keep "$A_ENV" SMARTFOOD_DISPATCH_MAX_ATTEMPTS)"; A_SDMAX="${A_SDMAX:-12}"
A_SDBASE="$(keep "$A_ENV" SMARTFOOD_DISPATCH_RETRY_BASE_SECONDS)"; A_SDBASE="${A_SDBASE:-5}"
A_SDCEIL="$(keep "$A_ENV" SMARTFOOD_DISPATCH_RETRY_MAX_SECONDS)"; A_SDCEIL="${A_SDCEIL:-60}"
A_SDLEASE="$(keep "$A_ENV" SMARTFOOD_DISPATCH_LEASE_SECONDS)"; A_SDLEASE="${A_SDLEASE:-60}"
...
WEB_CONCURRENCY=${A_WEBCONCURRENCY}
...
ALPHA_POS_ADMIN_EMAIL=${A_ADMIN_EMAIL}
ALPHA_POS_ADMIN_PASSWORD=${A_ADMIN_PASS}
...
OPENAI_REASONING_EFFORT=${A_OPENAIREASONING}
OPENAI_SEED=${A_OPENAISEED}
AI_TEMPERATURE=${A_AITEMPERATURE}
...
SMARTFOOD_AUTO_DISPATCH=${A_SAD}
SMARTFOOD_DISPATCH_MAX_ATTEMPTS=${A_SDMAX}
SMARTFOOD_DISPATCH_RETRY_BASE_SECONDS=${A_SDBASE}
SMARTFOOD_DISPATCH_RETRY_MAX_SECONDS=${A_SDCEIL}
SMARTFOOD_DISPATCH_LEASE_SECONDS=${A_SDLEASE}
SMARTFOOD_MAX_ITEM_QUANTITY=${A_SMAXQ}
...
eval "$A exec -T web python manage.py bootstrap_admin" || true
...
# 1) Provisioning ran automatically (license + POS application admin).
#    The generated application-admin credential is stored in $A_ENV.
#    No predictable Django /admin/ accounts are created automatically; run the
#    createsuperuser management command explicitly if that interface is needed.
...
#   CLOUD_SYNC_TOKEN = <DESKTOP_BRANCH_TOKEN from server .env>
...
# 3) Control center Django /admin/: run its createsuperuser management command
#    explicitly if an account is needed.
