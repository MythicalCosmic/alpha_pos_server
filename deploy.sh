...
# .env (never committed to git). Provider + model are safe to bake. Claude is the
# active provider; Gemini config is kept as a fallback.
...
# Claude (Anthropic) is active; the assistant gets read-only tool access to the
# live data. Gemini config is kept as a fallback (flip AI_PROVIDER to switch).
AI_PROVIDER=claude
