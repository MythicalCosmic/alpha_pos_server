...
env_value() { [ -f "$DIR/.env" ] && sed -n "s/^$1=//p" "$DIR/.env" | head -n1 || true; }
# With a real domain (ALPHAPOS_DOMAIN=example.com, remembered in .env) the
# public hosts are pos./admin./delivery./delivery-admin.<domain>. The nip.io
# names stay as aliases on the same routes, so the server is reachable by IP
# alone before DNS exists or if it ever breaks.
DOMAIN="${ALPHAPOS_DOMAIN:-$(env_value ALPHAPOS_DOMAIN)}"
IP_HOST="pos.${IP}.nip.io"
IP_DELIVERY_HOST="delivery.${IP}.nip.io"
IP_ADMIN_HOST="alpha-pos-admin.${IP}.nip.io"
IP_PANEL_HOST="new.${IP}.nip.io"
if [ -n "$DOMAIN" ]; then
    HOST="pos.${DOMAIN}"
    DELIVERY_HOST="delivery.${DOMAIN}"
    ADMIN_HOST="delivery-admin.${DOMAIN}"
    PANEL_HOST="admin.${DOMAIN}"
else
    HOST="$IP_HOST"
    DELIVERY_HOST="$IP_DELIVERY_HOST"
    ADMIN_HOST="$IP_ADMIN_HOST"
    PANEL_HOST="$IP_PANEL_HOST"
fi
# "a, b" when a domain host has a nip.io alias, else just the host.
site() { [ "$1" = "$2" ] && echo "$1" || echo "$1, $2"; }
...
CONTROL_URL="${LICENSE_CONTROL_CENTER_URL:-$(env_value LICENSE_CONTROL_CENTER_URL)}"
CONTROL_URL="${CONTROL_URL:-https://control.${IP}.nip.io}"
...
SDMAX="$(keep SMARTFOOD_DISPATCH_MAX_ATTEMPTS)"; SDMAX="${SDMAX:-12}"
SDBASE="$(keep SMARTFOOD_DISPATCH_RETRY_BASE_SECONDS)"; SDBASE="${SDBASE:-5}"
SDCEIL="$(keep SMARTFOOD_DISPATCH_RETRY_MAX_SECONDS)"; SDCEIL="${SDCEIL:-60}"
SDLEASE="$(keep SMARTFOOD_DISPATCH_LEASE_SECONDS)"; SDLEASE="${SDLEASE:-60}"
...
AIDEADLINE="$(keep AI_REQUEST_DEADLINE_SECONDS)"; AIDEADLINE="${AIDEADLINE:-150}"
AITOOLS="$(keep AI_MAX_TOOL_ITERATIONS)"; AITOOLS="${AITOOLS:-8}"
AIFALLBACKS="$(keep AI_FALLBACK_PROVIDERS)"; AIFALLBACKS="${AIFALLBACKS:-gemini,claude,openai}"
# The public POS host stays FIRST in ALLOWED_HOSTS: the redeploy helpers read it.
EXTRA_HOSTS=""; EXTRA_ORIGINS=""
if [ "$HOST" != "$IP_HOST" ]; then
    EXTRA_HOSTS=",${IP_HOST}"; EXTRA_ORIGINS=",https://${IP_HOST}"
fi
cat > "$DIR/.env" <<EOF
# Generated by deploy.sh — Alpha POS server edition
DEBUG=False
SECRET_KEY=${SECRET}
ALPHAPOS_DOMAIN=${DOMAIN}
ALLOWED_HOSTS=${HOST}${EXTRA_HOSTS},${IP},localhost,127.0.0.1
CSRF_TRUSTED_ORIGINS=https://${HOST}${EXTRA_ORIGINS}
CORS_ALLOW_ALL=True
...
SMARTFOOD_DISPATCH_MAX_ATTEMPTS=${SDMAX}
SMARTFOOD_DISPATCH_RETRY_BASE_SECONDS=${SDBASE}
SMARTFOOD_DISPATCH_RETRY_MAX_SECONDS=${SDCEIL}
SMARTFOOD_DISPATCH_LEASE_SECONDS=${SDLEASE}
SMARTFOOD_MAX_ITEM_QUANTITY=${SMAXQ}
COURIER_AUTO_ASSIGN=${CAA}
YANDEX_GEOCODER_KEY=${YGEO}
# --- AI assistant (LLM) — server-side only ---
# The current provider/model and any explicitly approved fallback order are
# preserved with the API keys; no deployment silently changes AI data egress.
...
GEMINI_MODEL=${GMODEL}
GEMINI_API_KEY=${GAPI}
LLM_CONNECT_TIMEOUT_SECONDS=${LLMCONNECT}
LLM_READ_TIMEOUT_SECONDS=${LLMREAD}
AI_REQUEST_DEADLINE_SECONDS=${AIDEADLINE}
AI_MAX_TOOL_ITERATIONS=${AITOOLS}
AI_FALLBACK_PROVIDERS=${AIFALLBACKS}
EOF
chmod 600 "$DIR/.env"
...
# Before DNS points the domain here, check the same route through its nip.io alias.
VERIFY_HOST="$HOST"
if [ "$HOST" != "$IP_HOST" ] && ! getent ahostsv4 "$HOST" | grep -qF "$IP "; then
    echo ">> ${HOST} does not resolve to ${IP} yet; checking the route via ${IP_HOST}"
    VERIFY_HOST="$IP_HOST"
fi
bash "$DIR/deploy/verify_public_route.sh" "$DIR" "$VERIFY_HOST" "$SHA"
