"""Business-day windows, dashboards, statistics,
...
class TestBusinessDayHelper:
    def test_close_boundary_and_quiet_gap_labels(self):
        from base.services.business_day import business_date
        assert business_date(_aware(2026, 3, 11, 2, 59)) == date(2026, 3, 10)
        assert business_date(_aware(2026, 3, 11, 3, 0)) == date(2026, 3, 11)
        assert business_date(_aware(2026, 3, 11, 6, 59)) == date(2026, 3, 11)

    def test_range_window_uses_canonical_open_and_close(self):
        from base.services.business_day import range_window
        lo, hi = range_window(date(2026, 3, 10), date(2026, 3, 10))
        assert lo == _aware(2026, 3, 10, 7, 0)
...
class TestDashboardBusinessWindow:
    def test_after_midnight_order_counts_to_previous_business_day(self):
        ...
        # 01:30 on the 11th is before the close of the 10th's service.
...
class TestOrderStatsParseDate:
    def test_parse_date_anchors_to_business_start(self):
        ...
        # Inclusive end is the last microsecond before the following 03:00 close.
...
class TestProductAnalytics:
    ...
    def test_trends_bucket_by_business_day(self):
        ...
        # 01:30 on the 11th belongs to the 10th's service.
...
class TestBusinessDayStartExposed:
    ...
    def test_auth_me_fallback_keeps_canonical_open(
        self, admin_session, monkeypatch,
    ):
        from base.services import business_day

        def unavailable():
            raise RuntimeError('business-day helper unavailable')

        monkeypatch.setattr(business_day, 'business_day_start', unavailable)
        resp = Client().get(
            '/api/admins/auth-me',
