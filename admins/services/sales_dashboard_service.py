...
def _series(d_from, d_to):
    ...
    from base.services.business_day import business_day_start, range_window
    ...
    for created_at, total, method, otype, is_paid, status in (
        ...
        ).values_list('created_at', 'total_amount', 'payment_method',
...
def sales_dashboard(range_token=None, date_from=None, date_to=None):
    ...
    return {
        ...
        'dayLabels': [d.isoformat() for d in days],
        'HM_DAYS': HM_DAYS,
        'HM_HOURS': HM_HOURS,
        'heatMatrix': heat,
        'channelDays': [{
