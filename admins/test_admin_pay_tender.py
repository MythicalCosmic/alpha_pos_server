...
def test_split_payments_write_lines_and_roll_up_to_mixed():
    ...
    _, st = AdminOrderService.mark_as_paid(o.id, payments=[
        {'method': 'CASH', 'amount': '35000'}, {'method': 'UZCARD', 'amount': '18000'}])
    assert st == 200
    o.refresh_from_db()
    assert o.payment_method == 'MIXED'
    assert _lines(o) == [('CASH', D('35000')), ('UZCARD', D('18000'))]
...
def test_short_payment_rejected():
    c = _cashier()
    o = _unpaid_order(c, 10000)
    _, st = AdminOrderService.mark_as_paid(o.id, payments=[{'method': 'CASH', 'amount': '5000'}])
    assert st == 422


def test_unpay_removes_lines():
    c = _cashier()
    o = _unpaid_order(c, 40000)
    AdminOrderService.mark_as_paid(o.id, payment_method='CASH')
    assert _lines(o) == [('CASH', D('40000.00'))]
    _, st = AdminOrderService.mark_as_paid(o.id, payment_method='CASH')  # already paid
    assert st == 400
    _, st = AdminOrderService.mark_as_unpaid(o.id)
    assert st == 200
    assert _lines(o) == []          # tender lines reversed, no stale rows
...
def test_settlement_equals_revenue_for_admin_paid_orders():
    ...
    AdminOrderService.mark_as_paid(o1.id, payment_method='CASH')
    ...
    assert t['CASH'] == D('135000')      # 100,000 + the 35,000 cash leg of the split
    assert t['HUMO'] == D('60000')
